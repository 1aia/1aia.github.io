...
then
    ...
    sudo sysctl -w vm.nr_hugepages=8 ; sudo echo "* soft memlock 262144" >> /etc/security/limits.conf ; sudo echo "* hard memlock 262144" >> /etc/security/limits.conf ; echo "." > /root/startup/wd/initdone ; reboot
fi
